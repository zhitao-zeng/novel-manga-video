...
- **提交任务不重复计费**：创建任务的请求只在"肯定没建成"时重试，即连不上服务或返回 429。读超时、连接中途断开、网关 504 都可能已经建了任务，所以不再重发：视频在 `task.json`、异步图片在它的 `task.json`、同步图片在 `*.unconfirmed.json` 里记下 `submit_uncertain_at`。之后同一个请求**一直不会自动重发**（以前是 30 分钟后重发，原任务若已受理就又建了一个），要有人核对过账单，再用 `thin_batch.py --resubmit-unconfirmed` 重跑（环境变量 `NOVEL_RESUBMIT_UNCONFIRMED=1`）。渲染器的限流退避也不会重发它。以前通用重试器会立刻再建一个任务，本地只记住第二个 ID。
...
- **付费车道重拍要人批**：`thin_batch.py --retake-failed` 给成片里没过语音门的段新拍两次（缓存里的失败不占次数），只在人批准的那一批里用，每次新拍都要付费。
