...
**资源池**：`configs/pipeline.json` 里 `base_url: "pool"` 的键（`h3pool`）不绑定哪台机器，每段提交时从资源池里取一个有空槽的实例。成员有两类：`configs/h3_pool.json` 里的常驻实例，和夜班（`/mnt/disk1/zengzhitao/h3-night-shift`）租到、`phase=active` 的实例。夜班实例在租约到期前 `drain_minutes`（15 分钟）停止接新段。每个实例有 `slots` 个槽（flock 锁文件）。实例可用要同时满足两条：`/health` 有应答；在夜班控制器每分钟一次的 nvidia-smi 巡检里，它的服务占着 GPU，而且没有出现"有排队任务、GPU 却连续 3 次空闲"。不可用的实例会写冷却文件，所有进程共同遵守；一段在某个实例上卡住超过 15 分钟，就换一个实例。
...
- 修正（`review_feedback.json`）写在成片之后，或者 H3 车道上英文提示词变了，这一集就算过期（`stale`），会重新渲染，但只有提示词变了的段会重新生成。
