...
**语音门失败补拍一次**：快速档原本一段只出一次，台词没念出来这段就带着缺口交付（46 集里 14 次）。现在快速档最多两次，第二次只在语音门失败时发生，且提示词带上 RETRY_SUFFIX 要求把台词说清楚；按失败率算成本约 +8%。
...
- 提示词里不写 `@音频N` 绑定，模型不看它，也不看音频顺序：它听参考音频，按画面里的人物（性别、年龄）去匹配。异性别的双人段两条都带、各归各位；同性别的双人段模型只能猜，可能互换。
