...
- **提交任务不重复计费**：创建任务的请求只在"肯定没建成"时重试，即连不上服务或返回 429。读超时、连接中途断开、网关 504 都可能已经建了任务，所以不再重发：`task.json` 记下 `submit_uncertain_at`，30 分钟内不再发同一个请求，渲染器的限流退避也不会重发它。以前通用重试器会再建一个任务，本地只记住第二个 ID。
- **单阶段超长就拆开**：一个阶段估计超过片段上限时，打包器按台词把它拆成几个阶段，每个都放得进一段；单句台词本身超长的，先在句末切开，再不行在逗号处切。拆出的后几段写"与上一段同一画面、同一站位继续"，台词一句不丢、顺序不变。以前是把请求时长硬压到上限，星海 484 的 `clip_05` 估计 71 秒只请求了 15 秒，台词缺了 45%。**已有的计划不会自动重新打包**：重打包会改变片段编号，整集重渲，付费的书要人定。
