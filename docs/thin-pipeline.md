...
- **快速档** `profile.json` 的 `tier: fast`（或 `--tier fast`）：规划保留一个 400 字以内的短思考步（去掉后首稿质量大跌，返修反而更多）、最多返修两次、目标按字数放大（每 3000 字约 60 秒，上限 100 秒）、8 个区段都要带到、漏引区段自动记为跳过、超长只记警告、群消息超长截断；每角色一张参考图；480p、语音门不重生成；无人值守时只审卡不审片；圣经每集增长一次（`thin_batch.GROW_STRIDE`，原为每 5 集；跳过会让角色在首次登场那一集进不了圣经，雾月实测半数角色平均晚 5.7 章、67 集已出片的首秀缺人）。质量档就是默认值。两档共用圣经、卡片和目录，可以先用快速档粗剪整本，再对挑出的集 `--tier quality --rerender` 精修。
...
- **静音只检查正文**：薄线生成的静音片尾卡不计入连续静音和静音占比。渲染器记录实际生成的片尾卡时长（`assembly.silent_outro_seconds`），质检从成片尾部排除这段，正文尾部的停顿仍计入；4.5 秒连续静音、35% 静音占比的门槛不变，黑场、音量、冻结画面及语音门也不变。旧成片用 `PYTHONPATH=src:scripts .venv/bin/python scripts/recheck_silence_thin.py --novel-dir outputs/wuyue --apply` 单独复检两项静音检查：核对保留的片尾卡、合成末段和当时的渲染设置，备份原报告后更新 QC 和薄线报告；不生成、不重合成、不改视频，也不重置渲染次数。去掉 `--apply` 可预检。

**进度页面**：`scripts/status_server.py` 的实时页（`/`）和看板（`/board`）共用 `thin_runs.episode_status`。完成量、今日产量和速度只计当前质检合格的成片；视频文件总数另列，未过质检、过期计划、生成失败、待审查和审查失败分别展示，并可展开查看待处理章节。出现需要人工处理的章节时暂停总完成时间预测。日期按当前成片文件最近合成时间统计，重做会改变日期，不代表历史首次出片量。审查报错不计入通过率分母，旧版本的审查结果不计入当前质量；实时页每 20 秒刷新，看板每 5 分钟刷新。模型汇总按最近一次渲染日志中的模型分组，可能包含沿用其他模型的缓存片段。
