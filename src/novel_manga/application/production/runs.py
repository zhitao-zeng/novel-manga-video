...
def episode_status(directory: Path, h3_lane: bool) -> str:
    ...
    if not plan_path.is_file():
        return "no_plan"
    from novel_manga.application.preparation.readiness import current_blocks
    ...
    if not final.is_file():
        return "pending"
    from novel_manga.application.profiles import blocking_clip_failures, assembly_gate_passed
...
def gate_failures(directory: Path) -> list[str]:
    ...
    try:
        ...
        return []
    from novel_manga.application.profiles import blocking_clip_failures
