...
from novel_manga.application.preparation.readiness import read
...
def brighten_dark_scene(video: Path, ranges: list[tuple[float, float]], directory: Path, cid: str) -> bool:
    ...
    from novel_manga.application.repair.history import archived_take
...
def technical_targets(directory: Path) -> dict[str, str]:
    ...
    from novel_manga.application.profiles import media_qc_ignores
...
def grant_changed_source_retry(directory: Path, before: dict, before_notes: dict, result: dict, extra_takes: int) -> list[str]:
    ...
    from novel_manga.application.repair.managed import generated_counts, generation_limit
    from novel_manga.application.repair.history import load, accepted_clip_material
...
def prepare(directory: Path, kind: str, *, extra_takes: int = 0) -> dict:
    if kind == 'entities':
        ...
        from novel_manga.application.repair.publication import write_artifacts
    ...
    if kind in {'managed','residual'}:
        from novel_manga.application.repair.managed import prepare as prepare_managed
    ...
    if kind in {'identity', 'residual'}:
        from novel_manga.application.preparation.readiness import collapsed_source_addresses
    ...
    if kind == 'source':
        ...
        return result
    if kind == 'plan':
        from novel_manga.application.packing.blocked import repair_episode
