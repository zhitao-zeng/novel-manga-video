...
class Settings:
    ...
    planner_beat_max_retries: int = 1
    planning_timeout_seconds: float = 600.0
    protected_lexicon: dict[str, str] = field(default_factory=dict)
    media_workers: int = 2
    video_workers: int = 2
    ...
    def from_env(
        ...
        protected_lexicon_raw = os.getenv("NOVEL_PROTECTED_LEXICON_JSON", "{}")
        try:
            protected_lexicon = json.loads(protected_lexicon_raw)
        except json.JSONDecodeError as error:
            raise ValueError("NOVEL_PROTECTED_LEXICON_JSON must be a JSON object") from error
        if not isinstance(protected_lexicon, dict) or not all(
            isinstance(key, str) and isinstance(value, str)
            for key, value in protected_lexicon.items()
        ):
            raise ValueError(
                "NOVEL_PROTECTED_LEXICON_JSON must map ASR spellings to canonical terms"
            )
        settings = cls(
            provider=provider,
            ...
            planner_beat_max_retries=int(
                os.getenv("NOVEL_PLANNER_BEAT_MAX_RETRIES", "1")
            ),
            planning_timeout_seconds=float(
                os.getenv("NOVEL_PLANNING_TIMEOUT_SECONDS", "600")
            ),
    ...
    def validate(self) -> None:
        ...
        if self.creative_profile != "short-drama-adaptive-v1":
            raise ValueError(
                "only the short-drama-adaptive-v1 production profile is executable"
            )
        ...
        if (
            self.admission_mode == "production"
            and self.provider != "mock"
            and self.planner_backend != "command"
        ):
            raise ValueError(
                "production short-drama-native requires the command planner"
            )
        ...
        if not 0 <= self.planner_beat_max_retries <= 3:
            raise ValueError(
                "NOVEL_PLANNER_BEAT_MAX_RETRIES must be between 0 and 3"
            )
        if not 1.0 <= self.planning_timeout_seconds <= 3600.0:
            raise ValueError(
                "NOVEL_PLANNING_TIMEOUT_SECONDS must be between 1 and 3600"
            )
        ...
        if self.final_audio_policy != NATIVE_DIALOGUE_POLICY:
            raise ValueError(
                "only native_dialogue is executable; legacy audio artifacts are read-only"
            )
        ...
        if "latentsync" in video_backend_identity:
            raise ValueError(
                "LatentSync is disabled; use a native-dialogue video backend"
            )
