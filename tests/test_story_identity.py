...
def reading(entities, mentions=(), relations=()):
    return {'policy': source_identity.POLICY, 'entities': entities, 'mentions': list(mentions), 'relations': list(relations), 'appearances': []}
...
def test_contextual_alias_cannot_merge_an_avatar_or_two_owners(tmp_path):
    ...
    aliases = identity_context_thin.effective_aliases(novel, context=context)
...
def test_renamed_lead_is_shared_by_aliases_and_speaker_rows(tmp_path):
    ...
    catalog = identity_store_thin.load_catalog(novel)
    assert identity_context_thin.effective_aliases(novel, context=context)['旧名字'] == '正式名'
    rows = source_identity.identity_rows(['正式名', '旧名字'], catalog.bible, '旧名字走过来。', context, catalog)
...
def test_short_word_is_only_a_person_when_source_reading_says_so(tmp_path):
    ...
    assert source_identity.identity_rows(['零'], catalog.bible, '温度降到了零。', nonperson, catalog)[0]['source_names'] == []
    person = reading({'e001': '零'}, [{'form': '零', 'entity_id': 'e001', 'presence': 'on_stage'}])
    assert source_identity.identity_rows(['零'], catalog.bible, '零推门进来。', person, catalog)[0]['source_names'] == ['零']
...
def test_source_quotes_and_ids_are_checked_before_saving(tmp_path):
    ...
    rows = source_identity.ground(answer, catalog, [{'text': '甲推门进来。'}])
    assert rows['mentions'][0]['source_quote'] == '甲推门进来。'
    answer['mentions'][0]['form'] = '原文没有的人'
    with pytest.raises(ValueError, match='does not occur'):
        source_identity.ground(answer, catalog, [{'text': '甲推门进来。'}])
...
def test_one_source_reading_is_reused_and_source_edits_invalidate_it(tmp_path, monkeypatch):
    ...
    first = identity_flow_thin.resolve_chapter(directory)
    assert identity_flow_thin.resolve_chapter(directory) == first and calls == ['chapter_identity_source']
    atomic_write_json(directory / 'segments.json', [{'segment_id': 'seg_1', 'text': '甲转身离开。'}])
    assert identity_store_thin.current_context(directory) == {}
    identity_flow_thin.resolve_chapter(directory)
...
def test_source_name_beats_a_conflicting_old_alias(tmp_path):
    ...
    r=source_identity.map_source_reading({'actors':[actor],'relations':[]},catalog,[{'text':'寒痕之龙，寒！'}])
...
def test_two_source_actors_do_not_collapse_through_a_legacy_alias(tmp_path):
    ...
    r=source_identity.map_source_reading({'actors':actors},identity_store_thin.load_catalog(novel),[{'text':'本体站在化身对面。'}])
...
def test_design_body_is_not_exposed_as_source_truth(tmp_path):
    ...
    ctx = identity_context_thin.prompt_context(directory)
...
def test_planner_and_packer_share_the_same_scoped_identity(tmp_path):
    ...
    ctx['inputs']=identity_store_thin.chapter_inputs(directory)
...
def test_reading_view_keeps_raw_source_and_does_not_rewrite_inserted_names(tmp_path):
    ...
    result = identity_context_thin.reading_segments(directory, context)
    assert result[0]['text'].count('〔身份注：') == 2
    assert '旧\n名〔身份注：用于人物指称时即正式名，同一实体〕' in result[0]['text']
    assert '化身〔' not in result[0]['text']
    assert identity_store_thin.read(directory / 'segments.json') == raw
...
def test_legacy_alias_not_supported_by_current_reading_is_only_a_retrieval_candidate(tmp_path):
    ...
    assert identity_context_thin.effective_aliases(novel, context=context) == {}
    prompt = identity_context_thin.prompt_context(directory, context=context)
...
def test_bad_auxiliary_mention_does_not_discard_the_grounded_actor():
    ...
    clean, dropped, pending = source_identity.clean_reading({'actors':[a]}, [{'text':'甲走进屋里。'}])
...
def test_readability_false_requires_confirmation_and_preserves_raw_evidence(tmp_path,monkeypatch,confirmed,extracted):
    ...
    if confirmed and extracted:
        result=identity_flow_thin.resolve_chapter(directory)
        assert result['mentions'][0]['source_quote']==raw[0]['text']
    else:
        expected=ValueError if confirmed else source_identity.UnreadableSource
        with pytest.raises(expected) as caught:
            identity_flow_thin.resolve_chapter(directory)
        if confirmed:
            assert not isinstance(caught.value,source_identity.UnreadableSource)
...
def test_empty_cached_extraction_is_repaired_not_reused(tmp_path, monkeypatch):
    ...
    atomic_write_json(directory / 'identity_context.json', {
        'policy': source_identity.POLICY, 'inputs': identity_store_thin.chapter_inputs(directory), 'source_actors': [], 'mentions': []})
    assert identity_store_thin.current_context(directory) == {}
    calls = []
    def ask(*args, **kwargs):
        calls.append(kwargs['name'])
    ...
    monkeypatch.setattr(model_client, 'ask_json', ask)
    result = identity_flow_thin.resolve_chapter(directory)
    assert source_identity.active_cast_names(result) == {'乙'}
    assert identity_flow_thin.resolve_chapter(directory) == result
...
def test_actorless_source_needs_positive_confirmation(tmp_path, monkeypatch, confirmed):
    ...
    if confirmed:
        result = identity_flow_thin.resolve_chapter(directory)
        assert result['actorless_confirmed'] and not source_identity.active_cast_names(result)
        assert identity_store_thin.current_context(directory) == result
        atomic_write_json(novel / 'bible_aliases.json', {'某称呼': '甲'})
        assert identity_flow_thin.resolve_chapter(directory)['actorless_confirmed']
    else:
...
def test_unmatched_actor_blocks_instead_of_borrowing_unrelated_cast():
    with pytest.raises(ValueError, match='新人'):
        source_identity.active_cast_names({'unmatched_actors': [{'name': '新人', 'kind': 'individual', 'presence': 'on_stage'}]})
