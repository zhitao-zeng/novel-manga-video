...
def test_offering_finds_the_bible_cast_by_name_pieces():
    ...
    assert entity_evidence.retrieval_keys("薇奥拉公主") == {"薇奥拉", "薇奥", "奥拉"} and "格雷" in entity_evidence.retrieval_keys("莱恩·格雷")
    forms = {e["id"]: {e["canonical"]} for e in entities}
    rows = {e["id"] for e in entity_evidence.offered(entities, forms, CH7, set())}
...
def test_locate_finds_quotations_despite_punctuation():
    assert entity_evidence.locate("原来作家小姐就是薇奥拉", CH7) == (CH7.index("原来"), CH7.index("薇奥拉。") + 3)
    assert entity_evidence.locate("早安，调查师", CH7) is not None
    assert entity_evidence.locate("莱恩", CH7) is None and entity_evidence.locate("这句不在", CH7) is None
...
def test_ground_keeps_only_backed_records():
    ...
    mentions, news, claims, dropped = entity_evidence.ground(7, CH7, raw)
...
def test_resolve_merges_reversibly_grows_the_bible_and_snapshots_the_scene(tmp_path, monkeypatch):
    root = novel(tmp_path)
    monkeypatch.setattr(ledger_judges, 'judge_claim', lambda claim, s, o: {"verdict": "supports", "why": "原文直说"})
    ledger = ledger_store.Ledger(root)
    summary = ledger_resolution.resolve_chapter(ledger, 7, CH7, RAW7, workers=1)
    ...
    ledger_resolution.resolve_chapter(ledger, 8, CH8, RAW8, workers=1)
    base = root / "entity"
    entities = {e["canonical"]: e for e in ledger_store.read_json(base / "entities.json", [])}
    ...
    assert ledger_store.read_json(base / "merges.json", []) == [{"claim": "c0007-01", "from": "e005", "into": "e002", "chapter": 7}]
    claims = {c["id"]: c for c in ledger_store.read_json(base / "claims.json", [])}
    assert claims["c0007-01"]["status"] == "accepted" and claims["c0008-01"]["status"] == "pending"   # kept from the reader: a person decides
    pages = ledger_views.pending_pages(root, {7: CH7, 8: CH8})   # the page a person reads before deciding
    ...
    shot = ledger_views.snapshot(root, 7)
    ...
    shot8 = ledger_views.snapshot(root, 8)
    assert shot8["must_not_reveal"] == [] and {(r["from"], r["to"]): r["bases"] for r in shot8["relations"]}[("艾琳娜", "薇奥拉公主")] == {"kinship_sibling": 1}
    assert len(index["relations"]) == 2 and any(d["what"] == "relation" for d in ledger_store.read_json(base / "dropped.json", []))
    # accept the body swap by hand: 艾琳娜 then acts through 薇奥拉's body
    claims["c0008-01"]["status"] = "accepted"
    ledger_store.write_json(base / "claims.json", list(claims.values()))
    cast8 = {r["name"]: r for r in ledger_views.snapshot(root, 8)["cast"]}
    ...
    assert ledger_resolution.decide(ledger_store.Ledger(root), "c0007-01", "rejected", "另一位作家")
    back = ledger_store.Ledger(root)
    ...
    assert [c["decision"] for c in ledger_store.read_json(base / "corrections.json", [])] == ["rejected"]
    ledger_resolution.resolve_chapter(back, 7, CH7, RAW7, workers=1)
    ...
    assert ledger_resolution.decide(back, "c0008-01", "accepted")
    assert {r["name"]: r["body"] for r in ledger_views.snapshot(root, 8)["cast"]}["艾琳娜"] == "e002"
...
def test_read_ahead_duplicates_are_reconciled_by_asking(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(ledger_judges, 'judge_same', same)
    ch19 = "多洛茜·路易斯推门进来。莱恩看着她。"
    ch20 = "路易斯小姐坐下了。莱恩点头。"
    ledger = ledger_store.Ledger(root)
    ledger_resolution.resolve_chapter(ledger, 19, ch19, {"chapter": 19, "mentions": [{"form": "多洛茜·路易斯", "entity": "NEW:多洛茜·路易斯", "kind": "proper", "presence": "on_stage", "evidence": "多洛茜·路易斯推门进来"}],
                                     "new_entities": [{"name": "多洛茜·路易斯", "kind": "person", "named": True, "description": "作家", "evidence": "多洛茜·路易斯推门进来"}], "claims": []}, workers=1)
    summary = ledger_resolution.resolve_chapter(ledger, 20, ch20, {"chapter": 20, "mentions": [{"form": "路易斯小姐", "entity": "NEW:路易斯小姐", "kind": "proper", "presence": "on_stage", "evidence": "路易斯小姐坐下了"}],
...
def test_a_bridging_form_asks_once_and_a_shared_surname_does_not(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(ledger_judges, 'judge_claim', lambda claim, s, o: {"verdict": "supports", "why": ""})
    asked = []
    monkeypatch.setattr(ledger_judges, 'judge_same', lambda a, b, fa, fb, note="": (asked.append((a["canonical"], b["canonical"], note)) or {"same": "unsure", "why": "笔名？"}))
    ledger = ledger_store.Ledger(root)
    ch19 = "多洛茜·路易斯推门进来。格雷先生点头。"
    ledger_resolution.resolve_chapter(ledger, 19, ch19, {"chapter": 19, "mentions": [
    ...
    ch74 = "艾琳娜·路易斯小姐坐下了。"
    ledger_resolution.resolve_chapter(ledger, 74, ch74, {"chapter": 74, "mentions": [
    ...
    assert claim["status"] == "pending" and ledger_store.read_json(root / "entity" / "asked.json", {}) == {"e003|e005": "unsure"}
    ledger_resolution.resolve_chapter(ledger, 75, "艾琳娜·路易斯又来了。", {"chapter": 75, "mentions": [
...
def test_a_form_on_a_stranger_record_is_relinked_and_a_label_never_absorbs_a_person(tmp_path, monkeypatch):
    ...
    (root / "story_bible.json").write_text(json.dumps({"characters": [
        ...
        ensure_ascii=False), encoding="utf-8")
    monkeypatch.setattr(ledger_judges, 'judge_claim', lambda claim, s, o: {"verdict": "supports", "why": "并列主语"})
    links = []
    monkeypatch.setattr(ledger_judges, 'judge_link', lambda form, evidence, assigned, options, forms: (links.append((form, assigned["canonical"], [o["canonical"] for o in options]))
    ...
    ledger_resolution.resolve_chapter(ledger, 20, text, raw, workers=1)
    assert links == [("路易斯小姐", "那位女士", ["多洛茜·路易斯"])]
    rows = {m["form"]: m["entity"] for m in ledger_store.read_json(root / "entity" / "mentions" / "ch_0020.json", [])}
    ...
    ledger_resolution.resolve_chapter(ledger, 21, "路易斯小姐又来了。", {"chapter": 21, "mentions": [
        {"form": "路易斯小姐", "entity": "e002", "kind": "proper", "presence": "on_stage", "evidence": "路易斯小姐又来了"}], "new_entities": [], "claims": []}, workers=1)
    assert len(links) == 1 and {m["form"]: m["entity"] for m in ledger_store.read_json(root / "entity" / "mentions" / "ch_0021.json", [])}["路易斯小姐"] == "e004"
    # a person accepting the rank claim keeps the name, not the label, as the survivor
    assert ledger_resolution.decide(ledger, claim["id"], "accepted") and ledger.by_id["e003"]["merged_into"] == "e005" and ledger.by_id["e005"]["canonical"] == "杰克·德昂"
...
def test_generic_bible_records_are_deduplicated_by_description(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(ledger_judges, 'ask_json', lambda parts, schema, name, max_tokens, **kwargs: {"same_ids": votes[parts[0]["text"].split("目标记录：[")[1][:4]], "why": "都是脑内女声"})
    ledger = ledger_store.Ledger(root)
    made = ledger_resolution.dedup_generic(ledger, workers=1)
    ...
    assert ledger_resolution.decide(ledger, "c0000-01", "accepted") and ledger.by_id["e003"]["merged_into"] == "e002"
...
def test_a_reading_made_before_its_people_existed_is_made_again(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(ledger_judges, 'extract_chapter', fake_extract)
    ch21 = "路易斯小姐坐下了。"
    early = ledger_extraction.extract(ledger, 21, ch21)                       # read ahead: the ledger ends at e004
    assert early["known_through"] == 4
    ledger_resolution.resolve_chapter(ledger, 19, "多洛茜·路易斯推门进来。", {"chapter": 19, "mentions": [
        {"form": "多洛茜·路易斯", "entity": "NEW:多洛茜·路易斯", "kind": "proper", "presence": "on_stage", "evidence": "多洛茜·路易斯推门进来"}],
        "new_entities": [{"name": "多洛茜·路易斯", "kind": "person", "named": True, "description": "作家", "evidence": "多洛茜·路易斯推门进来"}],
        "claims": []}, workers=1)
    monkeypatch.setattr(ledger_judges, 'judge_link', lambda *a, **k: {"entity": "UNCERTAIN", "why": "should not be asked"})
    summary = ledger_resolution.resolve_chapter(ledger, 21, ch21, early, workers=1)
    assert summary["reread"] == ["多洛茜·路易斯"] and calls[-1][0] == 21 and "多洛茜·路易斯" in calls[-1][1]
    assert {m["form"]: m["entity"] for m in ledger_store.read_json(root / "entity" / "mentions" / "ch_0021.json", [])} == {"路易斯小姐": "e005"}
    assert ledger_store.read_json(root / "entity" / "raw" / "ch_0021.json", {}).get("fresh") is True
...
def test_settling_asks_two_judges_with_the_whole_book(tmp_path, monkeypatch):
    ...
    root = novel(tmp_path)
    monkeypatch.setattr(ledger_judges, 'judge_claim', lambda claim, s, o: {"verdict": "insufficient", "why": "单章不够"})
    ledger = ledger_store.Ledger(root)
    ledger_resolution.resolve_chapter(ledger, 7, CH7, {**RAW7, "claims": [
    ...
    monkeypatch.setattr(ledger_judges, 'judge_settle', judge)
    ledger_resolution.decide(ledger, [c for c in ledger.claims if c["type"] == "lookalike"][0]["id"], "rejected", "人定的")   # a standing decision: not re-asked
    rows = ledger_resolution.settle_pending(ledger, workers=1)
...
def test_settling_never_lets_a_label_absorb_a_person_or_a_body_claim_without_a_name(tmp_path, monkeypatch):
    ...
    (root / "story_bible.json").write_text(json.dumps({"characters": [
        ...
        ensure_ascii=False), encoding="utf-8")
    monkeypatch.setattr(ledger_judges, 'judge_claim', lambda claim, s, o: {"verdict": "insufficient", "why": ""})
    monkeypatch.setattr(ledger_judges, 'judge_settle', lambda pack, rules: {"verdict": "supports", "why": "两个判官都同意"})
    ledger = ledger_store.Ledger(root)
    text = "红月秘女黛芙妮小姐来了。薇奥拉通过我的身体和你见面。"
    ledger_resolution.resolve_chapter(ledger, 46, text, {"chapter": 46, "mentions": [
    ...
    def judge(pack, rules):
        ...
        if rules is entity_contracts.SETTLE_RULES_BODY:
            return {"verdict": "contradicts", "why": "原文没点名身体主人"}
        return {"verdict": "supports", "why": "两个判官都同意"}
    monkeypatch.setattr(ledger_judges, 'judge_settle', judge)
    rows = {r["type"]: r for r in ledger_resolution.settle_pending(ledger, workers=1)}
...
def test_a_split_vote_goes_to_a_third_judge_and_nothing_stays_pending(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(ledger_judges, 'judge_claim', lambda claim, s, o: {"verdict": "insufficient", "why": ""})
    ledger = ledger_store.Ledger(root)
    ledger_resolution.resolve_chapter(ledger, 46, "红月秘女黛芙妮小姐来了。", {"chapter": 46, "mentions": [
        {"form": "黛芙妮", "entity": "e003", "kind": "proper", "presence": "on_stage", "evidence": "红月秘女黛芙妮小姐来了"}],
        "new_entities": [], "claims": [
    ...
    def judge(pack, rules):
        asked.append(rules)
        if rules is entity_contracts.SETTLE_RULES:
            return {"verdict": "supports", "why": "像"}
        if rules is entity_contracts.SETTLE_RULES_B:
            return {"verdict": "contradicts", "why": "不像"}
        return {"verdict": "contradicts", "why": "终审：分开处理"}
    monkeypatch.setattr(ledger_judges, 'judge_settle', judge)
    rows = {r["type"]: r for r in ledger_resolution.settle_pending(ledger, workers=1)}
    assert entity_contracts.SETTLE_RULES_TIEBREAK in asked
...
def test_merge_keeps_the_numerically_earlier_record(tmp_path, monkeypatch):
    ...
    ledger_resolution.resolve_chapter(ledger, 7, CH7, {**RAW7, "mentions": RAW7["mentions"][:2]}, workers=1)
...
def test_settling_closes_what_it_cannot_ask_about(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(ledger_judges, 'judge_claim', lambda claim, s, o: {"verdict": "insufficient", "why": ""})
    monkeypatch.setattr(ledger_judges, 'judge_settle', lambda pack, rules: {"verdict": "supports", "why": ""})
    ledger = ledger_store.Ledger(root)
    ...
    rows = {r["id"]: r for r in ledger_resolution.settle_pending(ledger, workers=1)}
