...
def test_bible_phrase_is_not_a_breakdown():
    ...
    assert not thin_review.BREAKDOWN.search(issue)
    assert thin_review.BREAKDOWN.search("手指非人，六指且穿模")


def test_extras_the_setting_did_not_list_are_not_missing():
    issue = "图5中出现了两名兜帽男性，这些角色在设定中未出现，属于多出的人物"
    assert not thin_review.MISSING.search(issue)
    assert thin_review.fix_tier(verdict(issue), bible()) == "optional"
...
def test_an_absent_named_character_is_must_fix():
    ...
    assert thin_review.MISSING.search(issue)
    assert thin_review.fix_tier(verdict(issue), bible()) == "must_fix"
    assert thin_review.fix_tier(verdict("莎拉完全缺失，未出场"), bible()) == "must_fix"
