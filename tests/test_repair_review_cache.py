...
def test_new_joint_recheck_can_add_a_real_error_to_an_old_pass(tmp_path):
    ...
    review.reconcile(d, evidence(current), {})
    updated, _ = review.reconcile(d, evidence(current, verdict='obvious', mode='joint'), {})
...
def evidence(current, *, verdict="fine", mode="all"):
    ...
    return {review.evidence_key(1, "clip_01", current["video"], current["take"]): record}
...
def test_imported_precise_pass_clears_false_candidate_without_a_model_call(tmp_path, monkeypatch):
    ...
    result, takes = review.reconcile(d, evidence(current), {})
    assert result["feedback"] == {}
    assert result["clips"]["clip_01"]["verify"]["verdict"] == "fine"
    assert review.missing_reviews(result, takes, "all") == []
...
def test_a_result_for_a_previous_take_cannot_clear_a_new_video(tmp_path):
    ...
    result, takes = review.reconcile(d, records, {})
    assert "verify" not in result["clips"]["clip_01"]
    assert takes["clip_01"]["video"] == str(new)
    assert review.missing_reviews(result, takes, "changed") == ["clip_01"]
...
def test_flash_is_a_candidate_until_locally_confirmed(tmp_path):
    ...
    result, takes = review.reconcile(d, evidence(current), flash)
    assert result["feedback"] == {}
    assert review.missing_reviews(result, takes, "flash") == ["clip_01"]
    assert json.loads((d / "episode_review.json").read_text())["clips"]["clip_01"]["flash_pending"]
    result, takes = review.reconcile(d, evidence(current, verdict="obvious", mode="confirm"), flash)
    assert result["feedback"] == {"clip_01": "甲只出现一次"}
    assert result["clips"]["clip_01"]["flash_checked"]
    assert review.missing_reviews(result, takes, "flash") == []


def test_repeated_local_scan_does_not_overwrite_current_precise_verdict(tmp_path):
    d, current = setup_episode(tmp_path)
    review.reconcile(d, evidence(current, verdict="obvious"), {})
    result, _ = review.reconcile(d, evidence(current), {})
    assert result["feedback"] == {"clip_01": "甲只出现一次"}


def test_source_recheck_replaces_old_confirmation_only_for_the_reviewed_take(tmp_path):
    d, current = setup_episode(tmp_path)
    review.reconcile(d, evidence(current, verdict='obvious', mode='confirm'), {})
    source = evidence(current, mode='source_confirm')
    next(iter(source.values()))['source_confirmed_at'] = '2026-09-15 14:40:00'
    result, _ = review.reconcile(d, source, evidence(current, verdict='obvious'))
    assert result['feedback'] == {}
    ...
    changed, _ = review.reconcile(d, source, {})
...
def test_legacy_final_gate_overrides_its_retained_first_judge_with_matching_evidence(tmp_path):
    ...
    assert result["feedback"] == {}
    assert review.inspection_counts(result, takes, ["clip_01"])["failed"] == 0
    assert review.inspection_counts(result, takes, ["clip_01"])["passed"] == 1
...
def test_changed_review_leaves_unchanged_legacy_passes_for_the_running_full_audit(tmp_path):
    ...
    result, takes = review.reconcile(d, {}, {})
    assert review.missing_reviews(result, takes, "changed") == []
    assert review.missing_reviews(result, takes, "all") == ["clip_01"]
...
def test_read_only_reconcile_does_not_change_a_claimed_episode(tmp_path):
    ...
    result, _ = review.reconcile(d, evidence(current), {}, write=False)
...
def test_inspection_does_not_count_old_candidates_as_confirmed_errors(tmp_path):
    ...
    result, takes = review.reconcile(d, {}, {})
    counts = review.inspection_counts(result, takes, ["clip_01"])
...
def test_inspection_counts_current_pass_failure_and_unknown_once_each(tmp_path):
    d, current = setup_episode(tmp_path)
    good, takes = review.reconcile(d, evidence(current), {})
    bad = review.verdict_from_record(next(iter(evidence(current, verdict="obvious").values())))
    good["clips"]["clip_02"] = bad
    takes["clip_02"] = current
    counts = review.inspection_counts(good, takes, ["clip_01", "clip_02", "clip_03"])
...
def test_inspection_requires_a_matching_take_even_if_a_verdict_remains(tmp_path):
    ...
    assert review.inspection_counts(result, takes, ["clip_01"])["unchecked"] == 1
    assert review.inspection_counts(result, {}, ["clip_01"])["unchecked"] == 1
