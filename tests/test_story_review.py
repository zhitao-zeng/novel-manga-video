...
def test_schema_asks_about_the_story():
    props = review_contracts.CLIP_SCHEMA["properties"]
    assert {"story_ok", "story_kind", "story_issue"} <= set(props)
    assert set(props["story_kind"]["enum"]) == set(review_contracts.STORY_KINDS)
    assert review_contracts.STORY_FATAL < set(review_contracts.STORY_KINDS)


def test_story_block_carries_the_passage_and_the_event():
    block = review_prompts.story_block({"prompt": PROMPT, "segment_ids": ["seg_4", "seg_5"]},
                                    {"seg_4": "她吻住了莱恩。", "seg_5": "“喵~”月下的小琥珀叫了一声。"})
    assert "她吻住了莱恩" in block and "小琥珀" in block and "薇奥拉环住莱恩的脖子" in block
    assert review_prompts.story_block({"prompt": "", "segment_ids": []}, {}) == "\n本段原文（剧情依据）：（无）\n"


def test_wrong_actor_or_missing_actor_is_a_retake():
    assert review_policy.fix_tier(verdict(), bible()) == "must_fix"
    assert review_policy.fix_tier(verdict(story_kind="原文中有动作的人物缺席"), bible()) == "must_fix"
    assert review_policy.fix_tier(verdict(story_kind="画面事件与原文不符"), bible()) == "ignore"
    assert review_policy.fix_tier(verdict(story_ok=True, story_kind="无问题", story_issue=""), bible()) == "ignore"
    # A scripted image does not excuse assigning its action to the wrong person.
    assert review_policy.fix_tier(verdict(scripted={"evidence": "x", "note": ""}), bible()) == "must_fix"
...
def test_the_correction_repeats_the_judge_in_the_books_terms():
    ...
    assert review_policy.flag_line("clip_05", verdict(), "must_fix").startswith("clip_05: 原文是薇奥拉")
...
def test_old_verdicts_without_story_fields_are_unchanged():
    ...
    assert review_policy.fix_tier(old, bible()) == "must_fix"
    assert "按原文修正剧情" not in review_policy.compose_feedback(old)
