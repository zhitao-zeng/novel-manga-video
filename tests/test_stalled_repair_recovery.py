...
@pytest.mark.parametrize('changed_words',[False,True])
def test_source_review_accepts_noop_picture_but_rejects_changed_dialogue(tmp_path,monkeypatch,changed_words):
    ...
    monkeypatch.setattr(repair,'repair_episode',lambda *a,**k:{'changed':[], 'proposal':{'plan':plan,'script':{'shots':[]},'notes':{},'changes':{}}})
